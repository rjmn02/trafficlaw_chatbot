...
export default function App() {
  ...
  const handleRetry = () => {
    if (!loading && lastQueryRef.current) {
      onAsk({ preventDefault: () => {} } as unknown as FormEvent);
    }
  };

  return (
    <main
      className="w-full h-screen flex relative overflow-hidden antialiased"
      style={{ backgroundColor: COLORS.background }}
    >
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 bg-black bg-opacity-30 z-30 pointer-events-none" />
      )}
      ...
      <section className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="w-full max-w-5xl mx-auto px-6 pb-6 flex-shrink-0 space-y-3">
          ...
          <p className="text-sm text-gray-500 mt-2 text-center">
            Note that PH Roadwise may make mistakes. It can only produce
            information based on 2025 Philippine traffic laws and regulations.
          </p>
        </div>
      </section>
